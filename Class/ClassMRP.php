	...
	for($i = 0; $i <= $m['kode_bahan_baku']; $i++){
		...
		$kode_bahan_baku = $m['kode_bahan_baku'];
			...
			for($i = $tanggal+1; $i <= $akhir; $i++)
			{
				...
				$sql3 = mysqli_query($this->con, "SELECT * FROM pre_order_bb WHERE tanggal = '$tgl_sr' AND bln = '$bln_sr' and status='0' and kode_bahan_baku = '$kode_bahan_baku'");
				...
				while($p = mysqli_fetch_array($sql))
				{
					...
					if($ph < $gr){
						...
						if ($ambilTgl <= $akhir){
							...
							$tgl2 = date('t-m-Y', strtotime(date('d-m-Y')));
						}
						...
						//INSERT KE DB MRP
						mysqli_query($this->con, "INSERT INTO porel(ID_mrp, kode_bahan_baku, rop, tanggal,tgl, bulan, status)VALUES('$ID_mrp','$kode_bahan_baku','$ROP','$tgl2','$periode1','$bulan','$status_porel')");
						
						mysqli_query($this->con, "UPDATE bahanbaku set rop = '$ROP' WHERE kode_bahan_baku = '$kode_bahan_baku'");
