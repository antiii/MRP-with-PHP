...
$pdf->Cell(6, 0.8, 'Kode Penjualan', 1, 0, 'C');
$pdf->Cell(5, 0.8, 'Nama Produk', 1, 0, 'C');
$pdf->Cell(5, 0.8, 'Kota', 1, 0, 'C');
...
if (isset($_GET['tanggal'])){
				$query=mysqli_query($con, "select * from penjualan  WHERE date BETWEEN '".$tgl_awal."' AND '".$tgl_akhir."'");
...
while($lihat=mysqli_fetch_array($query)){
	$pdf->Cell(1, 0.8, $no , 1, 0, 'C');
	$pdf->Cell(6, 0.8, $lihat['kode_penjualan'],1, 0, 'C');
	$pdf->Cell(5, 0.8, $lihat['nama_produk'],1, 0, 'C');
	$pdf->Cell(5, 0.8, $lihat['nama_toko'],1, 0, 'C');
	$pdf->Cell(3, 0.8, $lihat['jumlah_penjualan'], 1, 0,'C');
	$pdf->Cell(4, 0.8, $lihat['total_penjualan'], 1, 0,'C');
